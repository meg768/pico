...
def parseXML(xml, tags = None, debug = True):
...
def testRSS():
    ...
    print("Fetching...")
    response = request.get(expressen, headers = headers)

    print("Parsing...")
    rss = parseXML(response.text, filter)
    printRSS(rss)
...
def testFile():
    ...
    import json
    rss = parseXML('google.xml', filter)
    printRSS(rss)
